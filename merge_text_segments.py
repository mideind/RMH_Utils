...
def as_docs(line_stream):
    max_len = None
    last_file_id = None
    doc = []
    for line in line_stream:
        corpus_line = parse_line(line, max_len)
        if last_file_id is None:
            last_file_id = corpus_line.file_id

        if corpus_line.file_id != last_file_id and doc:
            yield doc
            doc = []
        doc.append(corpus_line)
        last_file_id = corpus_line.file_id

        if corpus_line.file_id == last_file_id:
            doc.append(corpus_line)
        else:
            if doc:
                yield doc
